...
(defn expand-bundled-ids
  ...
  (-> body
      (assoc  :kixi.datastore.metadatastore/bundled-files
              (into {}
                    (pmap #(hash-map % (get-file u d %)) bundled-ids)))
      (update :kixi.datastore.metadatastore/bundled-files
              (fn [files]
                (into {}
                      (pmap (fn [[id resp]]
                              (hash-map id
                                        (if (= 200 (:status resp))
                                          ...
                                          {:error (str "invalid status: " (:status resp))}))) files))))))
